...
class PaymentAcquirerAtom(models.Model):
    ...
    paytm_merchant_key = fields.Char('Merchent Key',
    ...
    def paytm_form_generate_values(self, values):
        ...
        paytm_values['reqHashKey'] = self.generate_checksum(paytm_values,
    ...
    def __id_generator__(self, size=6,
    ...
    def verify_checksum_by_str(self, param_str, merchant_key, checksum):
        ...
        salt = paytm_hash[-4:]
        calculated_checksum = self.generate_checksum_by_str(param_str,
                                                            merchant_key,
                                                            salt=salt)
...
class PaymentTransactionAtom(models.Model):
    ...
    def _paytm_form_get_invalid_parameters(self, data):
        ...
        if self.acquirer_reference and data.get(
                'mmp_txn') != self.acquirer_reference:
            invalid_parameters.append(
                ('ORDERID', data.get('ORDERID'), self.acquirer_reference))
